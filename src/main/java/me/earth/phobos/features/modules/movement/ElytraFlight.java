...
public class ElytraFlight extends Module
{
    ...
    public ElytraFlight() {
        ...
        this.speed = (Setting<Float>)this.register(new Setting("Speed", 1.0f, 0.0f, 10.0f, v -> this.mode.getValue() != Mode.FLY && this.mode.getValue() != Mode.BOOST && this.mode.getValue() != Mode.BETTER && this.mode.getValue() != Mode.OHARE, "The Speed."));
        this.vSpeed = (Setting<Float>)this.register(new Setting("VSpeed", 0.3f, 0.0f, 10.0f, v -> this.mode.getValue() == Mode.BETTER || this.mode.getValue() == Mode.OHARE, "Vertical Speed"));
        this.hSpeed = (Setting<Float>)this.register(new Setting("HSpeed", 1.0f, 0.0f, 10.0f, v -> this.mode.getValue() == Mode.BETTER || this.mode.getValue() == Mode.OHARE, "Horizontal Speed"));
    ...
    public void onSendPacket(final PacketEvent.Send event) {
        if (event.getPacket() instanceof CPacketPlayer && this.mode.getValue() == Mode.TOOBEE) {
            ...
            if (ElytraFlight.mc.player.isElytraFlying() && !ElytraFlight.mc.player.movementInput.jump && packet.getPitch(0.0F) < 1.0f) {
                //packet.pitch = 1.0f;
            }
        }
    }
    ...
    public void onDisable() {
        if (Feature.fullNullCheck() || ElytraFlight.mc.player.capabilities.isCreativeMode) {
            return;
        }
